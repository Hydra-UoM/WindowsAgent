...
vector<tuple<double, double,double,double,double>> procInfo;
int sampleCounter;          //counts the samples


//count aggregate values for cpu and memory for average calculations
void setProcInfo(double procID, double cpu, double mem){
	//search to find the process id 
	...
	procInfo.push_back(make_tuple(procID,cpu,mem,0,0));
}

//get average value for cpu 
double getProcCpuAvg(double procID){
	for (unsigned int i = 0; i < procInfo.size(); i++)
    {
		if(get<0>(procInfo[i]) == procID){
...
double getProcMemAvg(double procID){
	for (unsigned int i = 0; i < procInfo.size(); i++)
    {
		if(get<0>(procInfo[i]) == procID){
...
void getCpuStats()
{
	...
	vector<tuple<string, double,double,double, double, double,double,double>> finalData;
	...
	for (map<tstring, tuple<getProcessCpuUsage*, CPdhQuery*, CPdhQuery*, CPdhQuery*>>::iterator it = queries.begin();
		...
		try {
			...
			finalData.push_back(make_tuple(processName,procId, procMem, cpuUsage, avgMem, avgCPU,read,write));
			setProcInfo(procId, cpuUsage, procMem);
		}
	...
	sort(finalData.begin(), finalData.end(),
		[](tuple<string, double,double, double, double, double,double,double> a, tuple<string, double,double,double, double, double,double,double> b) {
		#ifdef SORT_CPU
	...
	for (auto d : finalData) {
		...
		oss << left << setw(10) << get<0>(d)
			...
			<< setw(5) << get<5>(d) << "%"
			<< setw(5) << get<6>(d) / (1024 * 8) << "Kb/s"
			<< setw(5) << get<7>(d) / (1024 * 8) << "Kb/s" << endl;
	}
	system("cls");
